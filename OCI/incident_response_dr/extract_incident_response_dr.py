"""
OCI Incident Response & Disaster Recovery (DR) Security Extractor.
...
class OCIIncidentResponseDRExtractor(BaseSecurityExtractor):
    def __init__(self, use_mock: bool = False):
        super().__init__(cloud_provider="OCI", domain_name="incident_response_dr", use_mock=use_mock)
    ...
    def extract_mock(self) -> List[SecurityRequirementItem]:
        return [
            SecurityRequirementItem(
                ...
                control_name="Enforce Immutable Retention Rules on OCI Object Storage Backup Buckets",
            ...
            SecurityRequirementItem(
                ...
                control_name="Configure OCI Full Stack Disaster Recovery (FSDR) for Tier-0 Applications",
...
if __name__ == "__main__":
    extractor = OCIIncidentResponseDRExtractor(use_mock=True)
    items = extractor.run()
    print(f"Extracted {len(items)} OCI Incident Response & DR items.")
