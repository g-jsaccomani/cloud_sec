"""
OCI Foundation & Landing Zone Security Requirements Extractor.
Extracts OCI Enterprise Landing Zone compartment hierarchy, Hub-and-Spoke
...
class OCIFoundationLandingZoneExtractor(BaseSecurityExtractor):
    def __init__(self, use_mock: bool = False):
        super().__init__(cloud_provider="OCI", domain_name="foundation_landing_zone", use_mock=use_mock)
    ...
    def extract_mock(self) -> List[SecurityRequirementItem]:
        return [
            SecurityRequirementItem(
                ...
                control_name="Enforce OCI Enterprise Landing Zone Compartment Architecture",
                ...
                recommended_value="Standardized OCI Landing Zone compartment structure with zero workloads in root compartment",
                status="COMPLIANT",
                severity="CRITICAL",
...
if __name__ == "__main__":
    extractor = OCIFoundationLandingZoneExtractor(use_mock=True)
    items = extractor.run()
    print(f"Extracted {len(items)} OCI Foundation Landing Zone items.")
