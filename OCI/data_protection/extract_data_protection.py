"""
OCI Data Protection Requirements & Configuration Extractor.
Extracts Object Storage bucket visibility & KMS encryption,
OCI Vault key rotation, Autonomous Database TLS rules, and Block Volume encryption.
"""
...
class OCIDataProtectionExtractor(BaseSecurityExtractor):
    def __init__(self, use_mock: bool = False):
        super().__init__(cloud_provider="OCI", domain_name="data_protection", use_mock=use_mock)
    ...
    def extract_mock(self) -> List[SecurityRequirementItem]:
        return [
            SecurityRequirementItem(
                ...
                control_name="Enforce Private Visibility on All OCI Object Storage Buckets",
            ...
            SecurityRequirementItem(
                ...
                remediation_notes="Assign OCI Vault MEK to remaining Dev/QA block volumes."
            ),
            SecurityRequirementItem(
                id="OCI-DATA-003",
                category="OCI Vault & KMS",
                control_name="Enforce Annual Rotation of OCI Vault Master Encryption Keys",
...
if __name__ == "__main__":
    extractor = OCIDataProtectionExtractor(use_mock=True)
    items = extractor.run()
    print(f"Extracted {len(items)} OCI Data Protection items.")
