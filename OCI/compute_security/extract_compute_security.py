"""
OCI Compute Security Requirements & Configuration Extractor.
Extracts OCI Shielded Instance posture, OKE Kubernetes private cluster settings,
OCI Bastion service usage, and OS Management patch compliance.
"""
...
class OCIComputeSecurityExtractor(BaseSecurityExtractor):
    def __init__(self, use_mock: bool = False):
        super().__init__(cloud_provider="OCI", domain_name="compute_security", use_mock=use_mock)
    ...
    def extract_mock(self) -> List[SecurityRequirementItem]:
        return [
            ...
            SecurityRequirementItem(
                ...
                control_name="Enforce OCI Bastion Service for SSH/RDP Sessions",
                ...
                current_value="OCI Bastion deployed in VCN 'vcn-prod-01'; 0 compute instances have public IPs",
                recommended_value="100% of administrative sessions routed via OCI Bastion with max 3-hour TTL",
                status="COMPLIANT",
                severity="CRITICAL",
...
if __name__ == "__main__":
    extractor = OCIComputeSecurityExtractor(use_mock=True)
    items = extractor.run()
    print(f"Extracted {len(items)} OCI Compute Security items.")
